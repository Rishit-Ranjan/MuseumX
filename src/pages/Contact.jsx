            ...
            <div className="grid gap-12 lg:grid-cols-[1.2fr_0.8fr] items-start">
                <div className="space-y-6">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div className="rounded-3xl border border-slate-800 bg-slate-950/90 p-6 shadow-xl shadow-black/10">
                            ...
                            <h2 className="text-xl font-semibold text-white mb-2">{labels.visitUs || 'Visit Us'}</h2>
                            <p className="text-slate-400 leading-relaxed">
                                {labels.address || 'Shyamla Hills, Bhopal, Madhya Pradesh, India'}
                            </p>
                        </div>
                        <div className="rounded-3xl border border-slate-800 bg-slate-950/90 p-6 shadow-xl shadow-black/10">
                            <div className="flex items-center justify-center w-12 h-12 rounded-2xl bg-amber-500/10 text-amber-300 mb-4">
                        ...
                        <div className="rounded-3xl border border-slate-800 bg-slate-950/90 p-6 shadow-xl shadow-black/10">
                            ...
                            <h2 className="text-xl font-semibold text-white mb-2">{labels.responseTime || 'Response Time'}</h2>
                            <p className="text-slate-400 leading-relaxed">{labels.responseTimeDesc || 'We usually reply within one business day.'}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-[2rem] border border-slate-800 bg-slate-950/90 p-8 shadow-2xl shadow-black/20">
                    <div className="mb-8 space-y-2">
                        <p className="text-sm uppercase text-amber-300 tracking-[0.24em]">{labels.sendMessage || 'Send a message'}</p>
                        <h2 className="text-3xl font-bold text-white">{labels.dropNote || 'Drop us a note'}</h2>
                        <p className="text-slate-400 leading-relaxed">{labels.dropNoteDesc || 'Share your question, suggestion, or collaboration idea and we’ll get back to you soon.'}</p>
                    </div>

                    <form onSubmit={handleSubmit} className="space-y-5">
                        <label className="block">
                            <span className="text-slate-300 text-sm font-medium">{labels.formName || 'Name'}</span>
                            <input
                                name="name"
                        ...
                        <label className="block">
                            <span className="text-slate-300 text-sm font-medium">{labels.formEmail || 'Email'}</span>
                            <input
                                type="email"
                                name="email"
                                ...
                                onChange={handleChange}
                                className="mt-2 w-full rounded-3xl border border-slate-800 bg-slate-900 px-4 py-3 text-slate-100 outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20"
                                placeholder={labels.formEmailPlaceholder || "you@example.com"}
                                required
                            />
                        </label>
                        <label className="block">
                            <span className="text-slate-300 text-sm font-medium">{labels.formMessage || 'Message'}</span>
                            <textarea
                                name="message"
                                ...
                                placeholder={labels.formMessagePlaceholder || "Tell us what you need help with"}
                                required
                            />
                        </label>
                        <button
                        ...
                        >
                            {labels.sendButton || 'Send Message'}
                        </button>
                        {submitted && (
                            <p className="text-emerald-400 text-sm mt-2">{labels.successMessage || 'Thank you! Your message has been recorded. We’ll get back to you shortly.'}</p>
                        )}
                    </form>
                </div>
            </div>
