          ...
          <div className="bg-slate-900 border border-slate-700 w-80 md:w-96 h-[500px] rounded-2xl shadow-2xl flex flex-col mb-4 overflow-hidden animate-in slide-in-from-bottom-5">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-950/50">
              {chatMessages.map((msg, i) => (
                <div key={i} className={`group flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} relative`}>
                  ...
                  <button onClick={() => deleteMessage(i)} className="opacity-0 group-hover:opacity-100 absolute -top-1 -right-1 bg-slate-800 text-slate-400 hover:text-red-400 rounded-full p-0.5 shadow-lg transition-all" title="Delete message">
                    <X size={10} />
                  </button>
                </div>
              ))}
